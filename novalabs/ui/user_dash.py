...
def student_dashboard(request: Request):
    ...
    with ui.header().classes('items-center justify-between'):
        ...
        with ui.row().classes('items-center gap-4'):
            ui.button('🌌 Cosmic View', on_click=lambda: ui.navigate.to('/cosmic')).props('flat color=white')
            ui.label(f"{user['first_name']} {user['last_name']}").classes('text-sm')
            ui.button('Logout', on_click=lambda: logout(request)).props('flat dense color=white')
                            ...
                            with ui.card().classes('hover:shadow-lg transition-shadow'):
                                ...
                                if progress:
                                    ...
                                    if status == 'unlocked':
                                        ui.button(
                                            'Start Lab',
                                            on_click=lambda _e, lab_ref=lab['ref']: start_lab(client, lab_ref)
                                        ).props('color=primary')
                                    elif status == 'in_progress':
                                        ui.button(
                                            'Continue',
                                            on_click=lambda _e, lab_ref=lab['ref']: launch_lab_internal(lab_ref)
                                        ).props('color=orange')
