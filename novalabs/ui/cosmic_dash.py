            ...
            with ui.row().classes('w-full items-center justify-between p-4'):
                ...
                with ui.row().classes('items-center gap-4'):
                    ...
                    ui.label().classes('rank-badge').bind_text_from(
                        ...
                        backward=lambda _: user.get('rank', 'Dabbler').upper()
                    )
    ...
    with ui.dialog() as dialog, ui.card().classes('w-[600px]').style(
        ...
        with ui.column().classes('gap-4 text-white'):
            ...
            with ui.row().classes('w-full justify-end gap-2'):
                ...
                # Debug: Print lab status and UI URL
                print(f"[DEBUG] Lab {lab['meta']['ref']}: status={lab['progress']['status']}, ui_url={lab['meta'].get('ui_url')}")
                ...
                if lab['progress']['status'] == 'unlocked':
                    ...
                    print(f"[DEBUG] Creating Launch Lab button for {lab['meta']['ref']}")
                    ui.button(
                        'Launch Lab',
                        on_click=lambda _e, lab_ref=lab['meta']['ref']: launch_lab_internal(lab_ref, request)
                    ).props('color=primary')
...
def start_lab(client, lab_ref, dialog, request):
    """Start a lab and mark it as in-progress"""
    try:
        # Start the lab
        client.start_lab(lab_ref)
        ui.notify('Lab started! You can now launch it.', type='positive')

        dialog.close()
        # Navigate to lab page
        ui.navigate.to(f'/lab?lab_ref={lab_ref}')
    except SDKClientError as e:
        ui.notify(f'Error: {str(e)}', type='negative')


def launch_lab_internal(lab_ref: str, request: Request):
    """Launch lab using internal lab page"""
    ui.navigate.to(f'/lab?lab_ref={lab_ref}')


def launch_lab_ui(ui_url, client, request):
    """Launch the lab UI in a new window with authentication token (fallback)"""
    if not ui_url:
        ui.notify('Lab UI not available', type='warning')
        return

    # Get the JWT token from session
    session_data = session_manager.load_session(request)
    ...
    full_url = f'{ui_url}?{params}'

    # Debug logging
    print(f"[DEBUG] Launching lab UI: {full_url}")
    ui.notify(f'Opening lab at {ui_url}...', type='info')

    # Open lab in new tab with token
    ui.run_javascript(f'window.open("{full_url}", "_blank")')
